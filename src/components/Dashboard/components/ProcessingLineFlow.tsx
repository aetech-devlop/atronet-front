...
export const ProcessingLineFlow: React.FC<ProcessingLineFlowProps> = ({ 
  ...
  React.useEffect(() => {
    ...
    // Initialize with all equipment selected
    const allIds = new Set(newEquipment.map(eq => eq.id));
  ...
  const createNodesWithData = (equipment: Equipment[]) => {
    return equipment.map((eq, index) => {
      ...
      return {
        ...
        style: {
          background: updatedStatus === 'online' ? '#dcfce7' : updatedStatus === 'warning' ? '#fef3c7' : '#fee2e2',
          border: updatedStatus === 'online' ? '1px solid #16a34a' : updatedStatus === 'warning' ? '1px solid #d97706' : '1px solid #dc2626'
        }
      };
    });
  };

  // Get visible equipment (selected for flow display)
  const visibleEquipment = availableEquipment.filter(eq => selectedEquipmentIds.has(eq.id));
  
  // Initial nodes for the flow
  const initialNodes: Node[] = createNodesWithData(visibleEquipment);

  const initialEdges: Edge[] = visibleEquipment.length > 1 
    ? visibleEquipment.slice(0, -1).map((_, index) => ({
        id: `e${index}-${index + 1}`,
        source: visibleEquipment[index].id,
        target: visibleEquipment[index + 1].id,
        type: 'smoothstep',
      }))
    : [];
  ...
  React.useEffect(() => {
    const newNodes = createNodesWithData(visibleEquipment);

    const newEdges = visibleEquipment.length > 1 
      ? visibleEquipment.slice(0, -1).map((_, index) => ({
          id: `e${index}-${index + 1}`,
          source: visibleEquipment[index].id,
          target: visibleEquipment[index + 1].id,
    ...
    setNodes(newNodes);
    setEdges(newEdges);
  }, [visibleEquipment, language, processingData, setNodes, setEdges]);
                  ...
                  {availableEquipment.map((equipment) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <Badge variant={getStatusColor(updatedStatus)} className="text-xs">
