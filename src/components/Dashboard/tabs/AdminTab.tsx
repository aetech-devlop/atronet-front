...
export const AdminTab = () => {
  ...
  const [isDeleteIpConfirmOpen, setIsDeleteIpConfirmOpen] = useState(false);
  ...
  const createUserMutation = useCreateUser({
    onSuccess: (data) => {
      toast({
        title: language === 'ko' ? '사용자 생성 완료' : 'User Created',
        description: language === 'ko' 
          ? `${data.name} 사용자가 성공적으로 생성되었습니다.`
          : `User ${data.name} has been created successfully.`,
        duration: 3000,
      });
      queryClient.invalidateQueries({ queryKey: userKeys.all });
      setTimeout(() => closeUserModal(), 100); // 토스트가 보이고 나서 모달을 닫습니다
    },
    onError: (error) => {
      console.error('Error creating user:', error);
      toast({
        title: language === 'ko' ? '사용자 생성 실패' : 'Error Creating User',
        description: language === 'ko' 
          ? '사용자 생성 중 오류가 발생했습니다.'
  ...
  const updateUserMutation = useUpdateUser({
    onSuccess: (response) => {
      // response가 직접 User 객체임
      toast({
        title: language === 'ko' ? '사용자 정보 수정 완료' : 'User Updated',
        description: language === 'ko'
          ? `${response.name || '사용자'} 정보가 성공적으로 수정되었습니다.`
          : `User ${response.name || 'user'}'s information has been updated successfully.`,
        duration: 3000,
      });
      queryClient.invalidateQueries({ queryKey: userKeys.all });
      closeUserModal(); // 즉시 모달을 닫습니다
    },
    onError: (error) => {
      console.error('Error updating user:', error);
      toast({
        title: language === 'ko' ? '사용자 수정 실패' : 'Error Updating User',
        description: language === 'ko'
  ...
  const deleteUserMutation = useDeleteUser({
    onSuccess: () => {
      toast({
        title: language === 'ko' ? '사용자 삭제 완료' : 'User Deleted',
        description: language === 'ko'
          ? '사용자가 성공적으로 삭제되었습니다.'
      ...
      setTimeout(() => {
        ...
        setItemToDelete('');
      }, 100); // 토스트가 보이고 나서 다이얼로그를 닫습니다
    },
    onError: (error) => {
      console.error('Error deleting user:', error);
      toast({
        title: language === 'ko' ? '사용자 삭제 실패' : 'Error Deleting User',
        description: language === 'ko'
          ? '사용자 삭제 중 오류가 발생했습니다.'
  ...
  const createIpAccessMutation = useCreateIpAccess({
    onSuccess: (data) => {
      toast({
        title: language === 'ko' ? 'IP 등록 완료' : 'IP Added',
        description: language === 'ko'
          ? `IP 주소 ${data.data?.address}가 화이트리스트에 추가되었습니다.`
          : `IP address ${data.data?.address} has been added to the whitelist.`,
        duration: 3000,
      });
      queryClient.invalidateQueries({ queryKey: ipAccessKeys.all });
      setTimeout(() => closeIpModal(), 100); // 토스트가 보이고 나서 모달을 닫습니다
    },
    onError: (error) => {
      console.error('Error creating IP access:', error);
      toast({
        title: language === 'ko' ? 'IP 등록 실패' : 'Error Adding IP',
        description: language === 'ko'
          ? 'IP 주소 등록 중 오류가 발생했습니다.'
  ...
  const deleteIpAccessMutation = useDeleteIpAccess({
    onSuccess: () => {
      toast({
        title: language === 'ko' ? 'IP 삭제 완료' : 'IP Removed',
        description: language === 'ko'
          ? 'IP 주소가 화이트리스트에서 제거되었습니다.'
          : 'IP address has been removed from the whitelist.',
        duration: 3000,
      });
      queryClient.invalidateQueries({ queryKey: ipAccessKeys.all });
      setTimeout(() => {
        setIsDeleteIpConfirmOpen(false);
        setItemToDelete('');
      }, 100); // 토스트가 보이고 나서 다이얼로그를 닫습니다
    },
    onError: (error) => {
      ...
      toast({
        title: language === 'ko' ? 'IP 삭제 실패' : 'Error Removing IP',
        description: language === 'ko'
          ? 'IP 주소 삭제 중 오류가 발생했습니다.'
  ...
  const handleUserSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 성공 시 처리는 mutation의 onSuccess에서 합니다
    } catch (error) {
  ...
  const handleConfirmDeleteUser = async () => {
    try {
      await deleteUserMutation.mutateAsync(itemToDelete);
      // 성공 시 처리는 mutation의 onSuccess에서 합니다
    } catch (error) {
      // 에러는 mutation의 onError에서 처리됩니다
      console.error('Error in handleConfirmDeleteUser:', error);
    }
  };
  ...
  const handleIpAllow = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    if (existingIp) {
      toast({
        title: language === 'ko' ? 'IP 등록 실패' : 'IP Registration Failed',
        description: language === 'ko' 
          ? '이미 등록된 IP 주소입니다.'
    ...
    try {
      ...
      const browserInfo = {
        browser: userAgent.match(/(chrome|safari|firefox|msie|trident(?=\/))\/?\s*(\d+)/i)?.[1] || '',
        os: userAgent.match(/windows|mac|linux|android|ios/i)?.[0] || '',
      };
      const deviceInfo = `${browserInfo.os.charAt(0).toUpperCase() + browserInfo.os.slice(1)} - ${browserInfo.browser}`;
        ...
        navigator.geolocation.getCurrentPosition(
          async (position) => {
            ...
            try {
              ...
              const locationString = locationData.display_name || `${position.coords.latitude}, ${position.coords.longitude}`;
              ...
              await createIpAccessMutation.mutateAsync(createData);
            } catch (error) {
              console.error('Error getting location details:', error);
              // 위치 정보 변환 실패 시 좌표만 사용
              const createData: IpAccessCreate = {
                user_id: "system",
                address: allowIpAddress,
      ...
      // 에러는 mutation의 onError에서 처리됩니다
      console.error('Error in handleIpAllow:', error);
    }
  };

  const deleteIp = (ipId: string) => {
    setItemToDelete(ipId);
    setIsDeleteIpConfirmOpen(true);
  };

  const handleConfirmDeleteIp = async () => {
    try {
      await deleteIpAccessMutation.mutateAsync(itemToDelete);
      // 성공 시 처리는 mutation의 onSuccess에서 합니다
    } catch (error) {
      // 에러는 mutation의 onError에서 처리됩니다
      console.error('Error in handleConfirmDeleteIp:', error);
    }
  };
  ...
  return (
    <div className="p-6">
      ...
      {/* Delete IP Confirm Dialog */}
      <ConfirmDialog
        open={isDeleteIpConfirmOpen}
        onOpenChange={setIsDeleteIpConfirmOpen}
        title={language === 'ko' ? 'IP 삭제' : 'Delete IP'}
        description={language === 'ko'
          ? '정말로 이 IP를 화이트리스트에서 제거하시겠습니까? 이 작업은 되돌릴 수 없습니다.'
          : 'Are you sure you want to remove this IP from the whitelist? This action cannot be undone.'}
        confirmText={language === 'ko' ? '삭제' : 'Delete'}
        cancelText={language === 'ko' ? '취소' : 'Cancel'}
        onConfirm={handleConfirmDeleteIp}
        variant="destructive"
      />
    </div>
  );
